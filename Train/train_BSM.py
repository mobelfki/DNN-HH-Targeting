...
def getArgs():
	...
	args.add_argument('-d', '--dropout_rate', action='store', default=0.21, help='Dropout rate')
...
def train(N_epochs,n_outputs,drop,learing_rate,_batch_size,_neurons):
	...
	es2       = EarlyStopping(monitor='val_loss', patience=4, verbose=1, mode='min', min_delta=0.0001)
...
#	pJJ_HH  = JJ[:,0]
	...
	for x in X:
		...
		if sumW_bkg < 0.001 or sumW_bkg_cut < 0.8:
			break;
...
def plot_Z_CutBased(HH,ttH,ZH,JJ,cat):
	...
	Scale_JJ  = [3.874, 4.245, 5.172, 3.397, 1.]; 
	"""
...
def main():
	...
	for i in range(0,1):
		...
		dirname = "BSM_model_Mbb_NLO_2btag77"
		...
		plt.hist(pHH, 50, histtype='step',  density='True')
		plt.hist(pttH,50, histtype='step', density='True')
		plt.hist(pZH, 50, histtype='step', density='True')
		plt.hist(pJJ, 50, histtype='step', density='True')
		plt.yscale('log')
		plt.xlabel('DHH')
		plt.ylabel('Event / 0.02')
		plt.legend(['HH','ttH','ZH','JJ'])
...
def Sim(X,Y,Z,W):
	...
	a = [math.log(X[i]*0.001711/(Y[i]*0.00114975+Z[i]*0.00172452+W[i]*51.823)) for i in range(0,X.shape[0])]
